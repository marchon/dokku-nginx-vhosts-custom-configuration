...
`fullbackup`       backup the databases, global configuration, and persistant data volumes
```bash
$ dokku fullbackup <app>            # Server side
$ ssh dokku@server fullbackup <app> # Client side
```

`volume:backup`    backup the contents of the /app path 
```bash
$ dokku backup <app>            # Server side
$ ssh dokku@server backup <app> # Client side
```

`volume:restore`   restore the contents of a tar file to the /app path  
```bash
$ dokku restore <app>            # Server side
$ ssh dokku@server restore <app> # Client side
```
...
## Credits 
   ...
   [dokku-nginx-vhosts-custom-configuration](https://github.com/neam/dokku-nginx-vhosts-custom-configuration);
   [Sandstorm Media Extensions for Dokku-Alt](https://github.com/sandstorm/dokku-alt-extensions); 
